...
const MyCustomMarker =(props)=>{
    return(
        <View style={{alignItems:"flex-end",justifyContent:"flex-end" , height:100}}>
            {console.log(props.item != undefined && props.item != null ? {uri : props.item.picture} : require('../assets/childface.png'))}
            <View style={{ position:"absolute",top:28,zIndex :2,right:17,height : 44,width : 44,borderRadius: 22,overflow: "hidden"}}>
                <Image style={{height: 45, width:45 }} source={props.item != undefined && props.item != null ? {uri : props.item.picture} : require('../assets/childface.png')} />
            </View>
